...
export function BookingWidget({ locationHints = [] }: { locationHints?: string[] }) {
  ...
  const leadMutation = useMutation({
    mutationFn: async (values: LeadFormValues) => {
      ...
      console.debug("[MarkleTravelBooking BookingWidget] submitting public lead", {
    ...
    onSuccess: (response) => {
      console.info("[MarkleTravelBooking BookingWidget] public lead created", response);
    ...
    onError: (error) => {
      ...
      console.error("[MarkleTravelBooking BookingWidget] public lead submission failed", error);
    ...
    >
      ...
      <header className="relative border-b border-white/12 pb-5">
        ...
        <p className="mt-2 max-w-md text-sm leading-relaxed text-[#F8FAFC]/62">
          This request binds to MarkleTravelBooking operations in real-time. Specialists compare partner fit across
      ...
      <footer className="relative mt-5 border-t border-white/12 pt-4">
        <Controller
          ...
          name="terms"
          rules={{
            validate: (value) => value || "Please accept MarkleTravelBooking contact terms",
          }}
          render={({ field }) => (
            <div>
              <label className="flex items-start gap-3 text-sm leading-relaxed text-[#F8FAFC]/70">
                ...
                />
                <span>
                  I authorize MarkleTravelBooking to contact me about this rental corridor and acknowledge the{" "}
